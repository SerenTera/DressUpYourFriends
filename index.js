...
/////Commands
	...
	command.add('dufake', () => {
		...
		command.message(`(Dressup)Ignore Fake equipment ${ignoreFake ? 'enabled' : 'disabled'}`);
	})
	...
	command.add('duid', namestri => {
		...
		if(players[namestri]) {
			...
			for(let i in customchat) idtext[packetname[i]] = playeridequip[i]
			command.message('(Dressup)Player '+namestri+' costume ids are: '+ JSON.stringify(idtext, null, "\t"))
			console.log('(Dressup)Player '+namestri+' costume ids are: '+ JSON.stringify(idtext, null, "\t"))
		}
	})
	...
	command.add('ducustom', (argu,name) => {
		if(argu.includes('json')) {
			fs.readFile(path.join(__dirname,'ducustom.json'),function(err,data) {
				if(err) {
					...
					Object.entries(data).forEach(([key,values]) => {
						if(isNaN(values)) delete data[key]
						else 
							data[key] = parseInt(values)
					})
					Object.assign(equips,data)
					command.message('(Dressup)Sucessfully modified saved equip. Try using dressup on someone or on yourself to check (type your own name)')
			})
		}
			...
			customstring=argu.replace(/[0-9\"\'\:\{\}]/g, '').replace(/ /g,'').toLowerCase().split(",") //fuck i dont wanna mess with this shit anymore
